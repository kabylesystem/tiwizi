...
function vocabLessonsFromCards(cardsAll, unitId, lessonSize) {
  ...
  for (let i = 0; i < cardsAll.length; i += lessonSize) {
    ...
    for (const c of cards) questions.push(mcWord(c, cardsAll));
    for (const c of cards) if (c.ex?.audioId) questions.push(listening(c.ex, frPool));
    for (const c of cards.slice(0, 4)) questions.push(order(c.ex));
    lessons.push({
      id: `${unitId}-l${li}`,
